...
<?php if(have_posts()): while(have_posts()): the_post();?>
    <article role="article" id="post_<?php the_ID()?>">
        <header>
            ...
            <h5>
              <em>
                <span class="text-muted author"><?php _e('By', 'f6st'); echo " "; the_author() ?>,</span>
        ...
        <section>
            ...
            <?php the_content( __( '&hellip; ' . __('Continue reading', 'f6st' ) . ' <i class="glyphicon glyphicon-arrow-right"></i>', 'f6st' ) ); ?>
        </section>
        <footer>
            <p class="text-muted" style="margin-bottom: 20px;">
                <i class="fa fa-folder-open-o"></i>&nbsp; <?php _e('Category', 'f6st'); ?>: <?php the_category(', ') ?><br/>
                <i class="fa fa-comment-o"></i>&nbsp; <?php _e('Comments', 'f6st'); ?>: <?php comments_popup_link(__('None', 'f6st'), '1', '%'); ?>
            </p>
        </footer>
    </article>
<?php endwhile; ?>

<?php if ( function_exists('f6st_pagination') ) { f6st_pagination(); } else if ( is_paged() ) { ?>
  <ul class="pagination">
    <li class="older"><?php next_posts_link('<i class="fa fa-arrow-left"></i> ' . __('Previous', 'f6st')) ?></li>
    <li class="newer"><?php previous_posts_link(__('Next', 'f6st') . ' <i class="fa fa-arrow-right"></i>') ?></li>
  </ul>
<?php } ?>
